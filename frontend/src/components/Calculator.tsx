...
export const Calculator: React.FC<CalculatorProps> = ({ isOpen, onClose, onApply, initialValue }) => {
    ...
    return (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
            ...
            <div className="relative bg-[#1C1C1E] w-full max-w-[280px] rounded-[32px] shadow-2xl overflow-hidden border border-white/10 animate-in zoom-in-95 duration-300">
                <div className="p-5 space-y-4">
                    <div className="flex items-center justify-between text-white/40">
                        <span className="text-[10px] font-black tracking-widest uppercase">Kalkulator</span>
                        <button onClick={onClose} className="p-2 hover:bg-white/5 rounded-xl transition-all">
                            <X className="w-4 h-4" />
                        </button>
                    </div>

                    <div className="bg-black/40 rounded-2xl p-4 text-right space-y-1 min-h-[80px] flex flex-col justify-end">
                        <div className="text-white/30 text-xs font-medium overflow-hidden text-ellipsis whitespace-nowrap">
                            {expression || '0'}
                        </div>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                        {buttons.map((btn, i) => (
                            <button
                                ...
                                onClick={btn.action}
                                className={`
                                    h-12 rounded-xl flex items-center justify-center text-base font-bold transition-all active:scale-90
                        ...
                        <button
                            ...
                            className="col-span-4 mt-1 h-12 bg-dagang-green text-white rounded-xl flex items-center justify-center gap-3 font-black text-xs shadow-xl shadow-dagang-green/10 hover:bg-dagang-green-light transition-all active:scale-[0.98]"
                        >
                            Pasang Rp {result.toLocaleString('id-ID')}
                        </button>
                    </div>
                </div>
            </div>
