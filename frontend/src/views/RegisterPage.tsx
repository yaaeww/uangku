...
export const RegisterPage = () => {
    ...
    const handleVerify = async (e: React.FormEvent) => {
        ...
        setError('');
        try {
            await AuthController.verifyOTP(formData.email, otp);
            setStep(4);
        } catch (err: any) {
    ...
    return (
        <div className="min-h-screen bg-dagang-cream text-dagang-dark font-sans flex flex-col md:flex-row">
            ...
            <div className="md:w-[42%] bg-gradient-to-br from-dagang-dark to-[#1e3d2b] p-[60px] md:p-[56px] flex flex-col justify-between relative overflow-hidden md:sticky md:top-0 md:h-screen">
                <div className="absolute top-[-100px] right-[-100px] w-[400px] h-[400px] bg-dagang-green/30 blur-[100px] rounded-full" />
                <div className="absolute bottom-[-80px] left-[-80px] w-[300px] h-[300px] bg-dagang-accent/5 blur-[80px] rounded-full" />
                ...
                <div className="relative z-10 my-10 md:my-0">
                    <h2 className="font-serif text-[36px] md:text-[42px] text-white leading-[1.15] mb-4">
                    ...
                    <div className="flex flex-col gap-4">
                        {[
                            ...
                            <div key={i} className="flex items-center gap-3.5">
                                <div className="w-7 h-7 bg-dagang-green/30 border border-dagang-green/50 rounded-full flex items-center justify-center text-[#4ade80] text-[12px] flex-shrink-0">
                ...
                <div className="bg-white/5 border border-white/10 rounded-[16px] p-5 md:px-6 flex items-center gap-4 relative z-10">
                    <div className="text-[28px]">🎁</div>
                    <div className="text-[13px] text-white/70">
                        <strong className="block text-[15px] text-white mb-0.5">Trial 7 Hari Gratis</strong>
                        Akses semua fitur lengkap. Tanpa kartu kredit. Batalkan kapan saja.
                    </div>
                </div>
            </div>

            {/* Right Panel - Form */}
            <div className="flex-1 p-6 md:p-[64px] flex flex-col justify-center bg-dagang-cream">
                <div className="max-w-[540px] mx-auto w-full">
                    ...
                    <div className="mb-10">
                        <h1 className="font-serif text-[38px] mb-2 leading-tight">Buat Akun Keluarga</h1>
                    ...
                    {[
                        ...
                        <Fragment key={s.num}>
                            <div className="flex items-center gap-2">
                                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[12px] font-bold transition-all ${step === s.num ? 'bg-dagang-green text-white' : step > s.num ? 'bg-dagang-green-pale text-dagang-green' : 'bg-black/5 text-dagang-gray'}`}>
                                    {step > s.num ? '✓' : s.num}
                                </div>
                                <div className={`text-[12px] font-semibold whitespace-nowrap ${step === s.num ? 'text-dagang-green' : 'text-dagang-gray'}`}>
                                    {s.label}
                                </div>
                            </div>
                            {i < 3 && (
                                <div className={`flex-1 h-[1px] mx-1 md:mx-3 max-w-[40px] md:max-w-[60px] ${step > s.num ? 'bg-dagang-green' : 'bg-black/10'}`} />
                            )}
                        </Fragment>
                    ))}
                        ...
                        <form className="space-y-5" onSubmit={handleRegister}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    <label className="text-[13px] font-semibold text-dagang-dark">Nama Lengkap</label>
                                ...
                                <div className="space-y-2">
                                    <label className="text-[13px] font-semibold text-dagang-dark">Nomor HP <span className="text-dagang-gray font-normal">(opsional)</span></label>
                                    <div className="relative">
                                        <Phone className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-dagang-gray/60" />
                                        <input type="tel" placeholder="08xx-xxxx-xxxx" className="w-full pl-11 pr-4 py-3 bg-white border-1.5 border-black/10 rounded-xl text-sm focus:border-dagang-green focus:ring-4 focus:ring-dagang-green/5 transition-all outline-none" />
                                    </div>
                                </div>
                            </div>

                            <div className="space-y-2">
                                <label className="text-[13px] font-semibold text-dagang-dark">Email</label>
                                <div className="relative">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    <label className="text-[13px] font-semibold text-dagang-dark">Password</label>
                                    <div className="relative">
                                ...
                                <div className="space-y-2">
                                    <label className="text-[13px] font-semibold text-dagang-dark">Konfirmasi Password</label>
                                    <div className="relative">
                                        <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-dagang-gray/60" />
                            ...
                            <div className="bg-dagang-green-pale border border-dagang-green/15 rounded-[16px] p-6 space-y-4">
                                ...
                                <div className="space-y-2">
                                    <label className="text-[13px] font-semibold text-dagang-dark">Nama Keluarga</label>
                                    <div className="relative">
                                        <Home className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-dagang-gray/60" />
                                        <input required type="text" placeholder="Keluarga Budi" value={formData.familyName} onChange={e => setFormData({ ...formData, familyName: e.target.value })} className="w-full pl-11 pr-4 py-3 bg-white border-1.5 border-dagang-green/20 rounded-xl text-sm focus:border-dagang-green focus:ring-4 focus:ring-dagang-green/5 transition-all outline-none" />
                                    </div>
                                </div>
                            </div>
                    ...
                    {step === 3 && (
                        <form className="space-y-6" onSubmit={handleVerify}>
                            <div className="text-center mb-8">
                                ...
                                <p className="text-dagang-gray text-sm mt-2">Kami telah mengirimkan kode OTP ke <strong>{formData.email}</strong></p>
                            </div>
                            {error && (
                                <div className="bg-red-50 border border-red-100 text-red-600 px-4 py-3 rounded-xl text-xs font-bold">
                                    {error}
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    className="w-full text-center text-3xl font-bold tracking-[0.5em] py-4 bg-white border-1.5 border-black/10 rounded-2xl focus:border-dagang-green focus:ring-4 focus:ring-dagang-green/5 outline-none transition-all"
                                />
                            </div>
